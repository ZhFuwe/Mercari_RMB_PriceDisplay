// ==UserScript==
// @name         Mercari 煤炉RMB价格显示
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  适配 Mercari 主站、Shops 页、搜索页、详情页，显示 RMB 价格，可修改汇率。
// @author       ZhFuwe
...
(function() {
    ...
    GM_addStyle(`
        ...
        .rmb-detail-text::before {
            content: '≈'; font-size: 0.8em; margin-right: 4px; color: #888; font-weight: normal;
        }
        ...
        .rmb-input-wrapper input { width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; }
        .rmb-save-btn { width: 100%; background: #d32f2f; color: white; border: none; padding: 10px; border-radius: 6px; cursor: pointer; margin-top: 15px;}
        
        /* 5. 布局修正 */
        [data-testid="item-cell"] > div > a, 
        .merItemThumbnail, 
        mer-item-thumbnail, 
        mer-item-thumbnail::part(container),
        a[data-testid="item-thumbnail-link"], /* Shops 页卡片 */
        div[data-testid="product-box"] /* Shops 搜索页卡片 */
    ...
    function calc(cleanNumStr) {
        ...
        if (!isNaN(num) && num > 0) return Math.floor(num * CONFIG.rate);
        return null;
    }
    ...
    function getRMB(text) {
        ...
        if (!str.includes('円') && !str.includes('¥') && !str.includes('￥')) {
             ...
             if (cleanStr.length > 0 && /\d/.test(str)) {
                 return calc(cleanStr);
             }
        }
    ...
    function mainLoop() {
        // 主站列表页
        document.querySelectorAll('[aria-label*="円"]:not([data-rmb-done])').forEach(el => {
            appendTagToElement(el, el.getAttribute('aria-label'));
        });
        ...
        // Shops 页
        const shopItems = document.querySelectorAll('a[data-testid="item-thumbnail-link"]:not([data-rmb-done])');
        shopItems.forEach(el => {
            appendTagToElement(el, el.innerText);
        });

        // Shops 搜索页
        const shopSearchItems = document.querySelectorAll('[data-testid="product-box"]:not([data-rmb-done])');
        shopSearchItems.forEach(el => {
            const priceNode = el.querySelector('[data-testid="product-thumbnail-item-price"]');
            if (priceNode) {
                appendTagToElement(el, priceNode.innerText);
            }
        });

        // 商品详情页
        const detailTargets = document.querySelectorAll(`
            [data-testid="price"]:not([data-rmb-done]),
            mer-price:not([data-rmb-done]),
            .item-price:not([data-rmb-done]),
            [data-testid="product-price"]:not([data-rmb-done])
        `);
    ...
    function createUI() {
        ...
        modal.innerHTML = `
            ...
            <div style="margin-bottom:15px">
                <label style="display:block;margin-bottom:5px;font-size:13px;color:#666">当前汇率</label>
                <div class="rmb-input-wrapper"><input type="number" id="rmb-rate-input" step="0.001"></div>
            </div>
        ...
        fab.onclick = (e) => {
            ...
            if (isOpen) document.getElementById('rmb-rate-input').value = CONFIG.rate;
        };
        ...
        document.getElementById('rmb-save-btn').onclick = () => {
            ...
            if (val > 0) {
                CONFIG.rate = val; GM_setValue('rate', val);
                location.reload();
            }
        };
